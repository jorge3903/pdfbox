...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            ...
            steps {
                sh 'mvn -f originalData/pdfbox/pdfbox -B -DskipTests clean'
                script {
		...
		request_proccesor= sh(returnStdout: true, script: "curl -i -o - --silent -X GET --header 'Accept: application/json' http://\${DV8_CONSOLE_IP}/preprocessor?directory=\${WORKSPACE}'&sourceCodePath=originalData/pdfbox'"); // we make the preprocessor request
